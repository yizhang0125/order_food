<?php
// Alternative PDF generation using TCPDF with real data
...
try {
    ...
    $pdf->SetTitle('Monthly Sales Report - Alternative');
    ...
    // Title
    $pdf->Cell(0, 12, 'MONTHLY SALES REPORT', 0, 1, 'C');
    $pdf->Ln(8);
    ...
    // Summary section with different layout
    $pdf->SetFont('helvetica', 'B', 13);
    $pdf->Cell(0, 8, 'SUMMARY', 0, 1, 'L');
    $pdf->Ln(8);
    
    // Summary data in a different format with real values
    $pdf->SetFont('helvetica', '', 11);
    $pdf->Cell(60, 8, 'Total Sales:', 0, 0, 'L');
    $pdf->SetFont('helvetica', 'B', 11);
    $pdf->Cell(0, 8, 'RM ' . number_format($total_sales, 2), 0, 1, 'L');
    
    $pdf->SetFont('helvetica', '', 11);
    $pdf->Cell(60, 8, 'Total Transactions:', 0, 0, 'L');
    $pdf->SetFont('helvetica', 'B', 11);
    $pdf->Cell(0, 8, number_format($total_transactions), 0, 1, 'L');
    
    $pdf->SetFont('helvetica', '', 11);
    $pdf->Cell(60, 8, 'Average Sale:', 0, 0, 'L');
    $pdf->SetFont('helvetica', 'B', 11);
    $pdf->Cell(0, 8, 'RM ' . number_format($average_sale, 2), 0, 1, 'L');
    
    $pdf->SetFont('helvetica', '', 11);
    $pdf->Cell(60, 8, 'Months Covered:', 0, 0, 'L');
    $pdf->SetFont('helvetica', 'B', 11);
    $pdf->Cell(0, 8, number_format($months_covered), 0, 1, 'L');
    
    $pdf->Ln(15);
    
    // Monthly breakdown with different layout
    $pdf->SetFont('helvetica', 'B', 13);
    $pdf->Cell(0, 8, 'MONTHLY BREAKDOWN', 0, 1, 'L');
    $pdf->Ln(8);
    
    // Monthly data in list format with real data
    $pdf->SetFont('helvetica', '', 11);
    foreach ($monthly_totals as $month_data) {
        ...
        $pdf->Cell(0, 8, $month_name . ':', 0, 1, 'L');
        $pdf->Cell(20, 8, '', 0, 0, 'L');
        $pdf->Cell(0, 8, 'Sales: RM ' . number_format($month_sales, 2) . ' | Transactions: ' . number_format($month_transactions) . ' | Average: RM ' . number_format($month_average, 2), 0, 1, 'L');
    }
    ...
    $pdf->Cell(0, 6, 'Generated by Restaurant Management System', 0, 1, 'C');
    $pdf->Cell(0, 6, 'This report includes all sales with applicable taxes and service charges', 0, 1, 'C');
    
    // Output PDF
    $pdf->Output('Monthly_Report_Alt_' . $start_date . '_to_' . $end_date . '.pdf', 'D');
