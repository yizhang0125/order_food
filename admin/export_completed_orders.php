...
try {
    ...
    if (empty($completed_orders)) {
        die('No completed orders found for the selected date range (' . $start_date . ' to ' . $end_date . ').');
    }
    ...
    // Set sheet title
    $sheet->setTitle('Completed Orders');

    // Add report header
    $sheet->setCellValue('A1', 'COMPLETED ORDERS REPORT');
    ...
    $sheet->setCellValue('A4', 'Total Orders: ' . count($completed_orders));
    ...
    $sheet->setCellValue('E' . $headerRow, 'Order Date');
    ...
    foreach ($completed_orders as $order) {
        ...
        $sheet->setCellValue('E' . $row, date('d M Y, h:i A', strtotime($order['created_at'])));
    ...
    $filename = 'completed_orders_' . $start_date . '_to_' . $end_date . '_' . date('His') . '.xlsx';
